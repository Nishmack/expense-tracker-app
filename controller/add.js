...
exports.getexpense = async (req, res, next) => {
  try {
    ...
    const totalexpense = await alldetails.count({
...
exports.downloadexpense = async (req, res) => {
  try {
    // Fetch expenses for the user from a service.userservices.js file
    const expenses = await UserServices.getExpenses(req);

    // Convert expenses to a JSON string.we cant write array to file .we can write string
    ...
    // Upload the JSON data to an S3 bucket and get the file URL.send it to frontend
    ...
    const fetchedData = await downloaddb.findAll({
      where: { userId: req.user.id },
    });
